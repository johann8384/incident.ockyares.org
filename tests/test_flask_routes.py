...
class TestFlaskRoutes:
    ...
    def test_health_check_success(self, client):
        ...
        with patch('app.db_manager.connect') as mock_connect, \
             patch('app.db_manager.close') as mock_close:
    ...
    def test_set_incident_location_success(self, client, sample_coordinates):
        ...
        with patch('app.Incident') as mock_incident_class:
            ...
            response = client.post('/api/incident/TEST-001/location',
    ...
    def test_set_search_area_success(self, client, sample_search_area):
        ...
        with patch('app.Incident') as mock_incident_class:
            ...
            response = client.post('/api/incident/TEST-001/search-area',
...
class TestIntegrationFlow:
    ...
    def test_complete_incident_workflow(self, client, sample_incident_data, 
                                      sample_coordinates, sample_search_area):
        ...
        with patch('app.Incident') as mock_incident_class:
            ...
            # 2. Set location
            response = client.post(f'/api/incident/{incident_id}/location',
                                 data=json.dumps(sample_coordinates),
                                 content_type='application/json')
            assert response.status_code == 200
            
            # 3. Set search area
            response = client.post(f'/api/incident/{incident_id}/search-area',
                                 data=json.dumps({'coordinates': sample_search_area}),
                                 content_type='application/json')
            assert response.status_code == 200
